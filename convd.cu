...
int main(int argc, char* argv[])
{
	...
	pixel *Pixel_out_cpu=(pixel*)malloc(num_pixels*sizeof(pixel));//store cpu output, global memory
	pixel *Pixel_out=(pixel*)malloc(num_pixels*sizeof(pixel));//store gpu output, global memory
	//storing the pixels as lexicographically
	pixel *Pixel = (pixel*)malloc(num_pixels*sizeof(pixel)); //store input image on global memory
	...
	cout<<" File reading time:"<<double(reading_file - kernel_generation)<<" sec"<<endl;//computed the reading time
	...
	time_t before_vertical_convolution =time(NULL);
	...
	cout<<" vertical_convolution time on gpu: "<<double(vertical_convolution - before_vertical_convolution)<<"sec"<<endl;
	...
	cout<<" horizontal convolution time on gpu:" <<double(horizontal_convolution-vertical_convolution)<<" sec"<<endl;
	...
	cout<<"File writing GPU: " <<double(end1-horizontal_convolution)<<" sec"<<endl;

	pixel *Pixel_tmp=vertical_conv_cpu(Pixel,img_wd, img_ht,kernel0,k);
	
	time_t vertical_convolution_cpu=time(NULL);

	Pixel_out=horizontal_conv_cpu(Pixel_tmp, img_wd, img_ht, kernel1, k);

	time_t horizontal_convolution_cpu=time(NULL);

	cout<<" vertical_convolution time on cpu: "<<double(vertical_convolution_cpu - end1)<<"sec"<<endl;

	cout<<" horizontal _convolution time on cpu: "<<double(horizontal_convolution_cpu-vertical_convolution_cpu )<<"sec"<<endl;
	//writing this to PPM file
	...
	cout<<" Saving the result CPU:"<<double(end-horizontal_convolution_cpu)<<" sec"<<endl;
